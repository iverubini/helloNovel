...
String [] subject = {"Orwell", "Huxley", "Michaella", "Cologne", "London", "He", "Winston", "Sophia", "Gertrude", "She"};
...
String [] names = {"Maria", "Laetitia", "Anna-Varney", "Oswald", "Orwell", "Pasadena", "Michaella", "Cologne", "London",
...
//  if (frameCount%2==0) {text (frameCount, 430, 615 );}
//  else                 {text (frameCount, 15, 615);}
  
  if (frameCount%2==0) {text (frameCount, 415, 615);}
  else                 {text (frameCount, 15, 615 );}
}
...
void simple_affirmation(){
  ...
  float loc = random (0, 9);
   ...
   stroke(244, 68, 46);
   fill(244, 68, 46);
  for (int i = 17; i < width; i = i + width / 2) {
    for (int ii = 4; ii < height ; ii = ii + height / 3) {
